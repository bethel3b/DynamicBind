...
def run_inference_for_complex(
    ...
    try:
        ...
        result = do(cmd, show=True)  # Show output to debug
...
def main(args):
    ...
    if args.results_dir is None:
        results_dir = os.path.join(output_dir, args.mode)
    else:
        results_dir = args.results_dir

    # Set up temporary directory
    if args.temp_dir is None:
        temp_dir = os.path.join(output_dir, "tmp", f"dynamicbind_benchmark_{args.mode}")
    else:
    ...
    if args.limit is not None:
        file_name_list = file_name_list[: args.limit]
    ...
    poses = args.poses
    if poses is None:
        poses = 3 if args.mode == "hts" else 40

    logger.info(
        f"Starting PDBbind benchmark with {total_complexes} protein-ligand complexes"
    )
    logger.info(f"Mode: {args.mode.upper()} ({poses} poses per complex)")
    ...
    for i, pdb_id in enumerate(file_name_list, 1):
        ...
        success = run_inference_for_complex(
            data_dir, pdb_id, temp_dir, results_dir, args.mode, args.poses
        )
    ...
    summary_path = os.path.join(results_dir, f"benchmark_summary_{args.mode}.csv")
    ...
    for pdb_id in file_name_list:
        ...
        if os.path.exists(affinity_file):
            try:
                ...
                df["complex_id"] = pdb_id
                df["mode"] = args.mode
                df["total_poses"] = poses
    ...
    if consolidated_results:
        ...
        consolidated_path = os.path.join(
            results_dir, f"consolidated_affinity_predictions_{args.mode}.csv"
        )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Run DynamicBind benchmark on PDBbind dataset"
    )
    parser.add_argument(
        "--mode",
        choices=["hts", "standard"],
        default="standard",
        help="Inference mode: hts (fast, 3 poses) or standard (thorough, 40 poses)",
    )
    parser.add_argument(
        "--poses",
        type=int,
        default=None,
        help="Number of poses to generate (overrides mode default)",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--data-dir",
        default="/NAS/data/betty_dev/DynamicBind/betty/pdbbind/data",
        help="Path to PDBbind data directory",
    )
    parser.add_argument(
        "--results-dir",
        default=None,
        help="Path to results directory (auto-generated if not provided)",
    )
    parser.add_argument(
        "--temp-dir",
        default=None,
        help="Path to temporary directory (auto-generated if not provided)",
    )
