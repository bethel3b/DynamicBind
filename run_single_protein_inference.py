...
parser = argparse.ArgumentParser(
    description="python run_single_protein_inference.py data/origin-1qg8.pdb data/1qg8_input.csv --header test"
)
...
parser.add_argument(
    "--samples_per_complex", type=int, default=10, help="num of samples data generated."
)
parser.add_argument(
    "--savings_per_complex",
    type=int,
...
parser.add_argument(
    "--relax_python",
    type=str,
    default="/home/zhangjx/anaconda3/envs/relax/bin/python",
...
parser.add_argument(
    "-p", "--paper", action="store_true", default=False, help="use paper version model."
)
...
parser.add_argument(
    "--hts", action="store_true", default=False, help="high-throughput mode"
)
...
if args.protein_path_in_ligandFile:
    if args.no_clean:
        ...
        ligandFile_with_protein_path = (
            f"./data/ligandFile_with_protein_path_{timestamp}.csv"
        )
...
if args.hts:
    ...
    if not args.no_inference:
        os.system("mkdir -p data")
        print("*" * 100)
        print("STAGE: ESM Embedding Preparation")
        print("*" * 100)
        cmd = f"{python} {script_folder}/datasets/esm_embedding_preparation.py --protein_ligand_csv {ligandFile_with_protein_path} --out_file data/prepared_for_esm_{header}.fasta"
        do(cmd)
        print("*" * 100)
        print("STAGE: ESM Embedding Extraction")
        print("*" * 100)
        cmd = f"CUDA_VISIBLE_DEVICES={args.device} {python} {script_folder}/esm/scripts/extract.py esm2_t33_650M_UR50D data/prepared_for_esm_{header}.fasta data/esm2_output --repr_layers 33 --include per_tok --truncation_seq_length 10000 --model_dir {script_folder}/esm_models"
        do(cmd)
        print("*" * 100)
    ...
    if not args.no_relax:
        print("*" * 100)
        print("STAGE: Relax Final Step Structure")
        print("*" * 100)
