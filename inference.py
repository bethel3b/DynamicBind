...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to a .csv file specifying the input as described in the README. If this is not None, it will be used instead of the --protein_path and --ligand parameters",
)
parser.add_argument(
    "--protein_path",
    type=str,
    default="data/dummy_data/1a0q_protein.pdb",
    help="Path to the protein .pdb file",
)
parser.add_argument(
    "--ligand",
    type=str,
    default="COc(cc1)ccc1C#N",
...
parser.add_argument(
    "--savings_per_complex", type=int, default=1, help="Number of samples to save"
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Use ODE formulation for inference",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Number of workers for creating the dataset",
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Use existing cache file, if they exist.",
)
...
test_dataset = PDBBind(
    ...
    if score_model_args.esm_embeddings_path is not None
    else None,
    require_ligand=True,
...
def predict_one_complex(
    ...
    final_data_list, data_list_step, all_lddt_pred, all_affinity_pred = (
        [],
    ...
    for i in range(int(np.ceil(len(data_list) / args.batch_size))):
        ...
        try:
            outputs = sampling(
                data_list=data_list[i * args.batch_size : (i + 1) * args.batch_size],
                model=model,
    ...
    ligand_pos = np.asarray(
        [
            ...
            for complex_graph in final_data_list
        ]
    )
    final_receptor_pdbs = []
    ...
    ##################################################
    # BETHEL - commented out this line
    ...
    ##################################################
    # BETHEL - rename output directory to unique_uid
    if "unique_id" in row.columns:
        unique_uid = row["unique_id"].values[0]
        # print(f"Unique UID: {unique_uid}")
        write_dir = os.path.join(args.out_dir, f"index_{true_idx}_{unique_uid}")
    os.makedirs(write_dir, exist_ok=True)
    ##################################################
    ...
    final_affinity_pred = np.minimum(
        (all_affinity_pred * all_lddt_pred).sum() / (all_lddt_pred.sum() + 1e-12), 15.0
    )
    ...
    for rank, order in enumerate(
        range(min(args.samples_per_complex, len(all_lddt_pred)))
    ):
        mol_pred = copy.deepcopy(lig)
        ligandFile = os.path.join(
            write_dir,
            f"step1_rank{rank + 1}_ligand_lddt{all_lddt_pred[order]:.2f}_affinity{all_affinity_pred[order]:.2f}.sdf",
        )
        ...
        pdbFile = os.path.join(
            write_dir,
            f"step1_rank{rank + 1}_receptor_lddt{all_lddt_pred[order]:.2f}_affinity{all_affinity_pred[order]:.2f}.{pdb_or_cif}",
        )
        save_protein(new_receptor_pdb, pdbFile)
        ligandFiles.append(ligandFile)
        pdbFiles.append(pdbFile)
        clash_scores.append(
            compute_side_chain_metrics(pdbFile, ligandFile, verbose=False)
        )

    re_order = np.argsort(
        scipy.stats.rankdata(-all_lddt_pred) + scipy.stats.rankdata(clash_scores) / 2.0
    )  # np.argsort(all_lddt_pred)[::-1]
    complete_affinity = pd.DataFrame(
        {
            "name": orig_complex_graph.name[0],
            "rank": np.arange(len(all_lddt_pred)) + 1,
            "lddt": all_lddt_pred[re_order],
            "affinity": all_affinity_pred[re_order],
        }
    )
    ...
    for rank, order in enumerate(re_order):
        os.rename(
            ligandFiles[order],
            ligandFiles[order].replace(f"step1_rank{order + 1}", f"rank{rank + 1}"),
        )
        os.rename(
            pdbFiles[order],
            pdbFiles[order].replace(f"step1_rank{order + 1}", f"rank{rank + 1}"),
        )
    if args.save_visualisation:
        for rank, order in enumerate(re_order[: args.savings_per_complex]):
            ...
            with open(
                os.path.join(write_dir, f"rank{rank + 1}_reverseprocess_data_list.pkl"),
                "wb",
            ) as f:
...
for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
    ...
    try:
        affinity_pred, complete_affinity = predict_one_complex(
            affinity_pred,
            df,
            orig_complex_graph,
            model,
...
affinity_pred_df = pd.DataFrame(
    {"name": list(affinity_pred.keys()), "affinity": list(affinity_pred.values())}
)
affinity_pred_df.to_csv(f"{args.out_dir}/affinity_prediction.csv", index=False)
pd.concat(all_complete_affinity).to_csv(
    f"{args.out_dir}/complete_affinity_prediction.csv", index=False
)
